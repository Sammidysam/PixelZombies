...
public class Manager {
	...
	public void loop(float mouseX, float mouseY){
		if(man == null){
			...
			if(makeGun)
				createNew(ID.STATICBULLET, locationX + (rand.nextInt(40) - 20), locationY + (rand.nextInt(40) - 20));
			if(makeGun)
				createNew(ID.STATICGUN, locationX, locationY);
			if(!makeGun)
				createNew(ID.STATICKNIFE, locationX, locationY);
							...
							for(int y = 0; y < knifeNumber; y++)
								if(knife[y].equipped && knife[y].equippedBy == ID.HUMAN)
									if(knife[y].equippedNumber == u)
										knife[y].reset();
							for(int y = 0; y < gunNumber; y++)
								if(gun[y].equipped && gun[y].equippedBy == ID.HUMAN)
									if(gun[y].equippedNumber == u)
										gun[y].reset();
	...
	private void humanLogic(){
		for(int i = 0; i < humanNumber; i++){
			if(!human[i].dead){
				...
				if(!human[i].goForWeapon){
					if(timekeeper.getTime() - human[i].time > 5000)
						human[i].goForWeapon = true;
				}
				human[i].move();
				if(human[i].zombieNear){
					createNewBullet(zombie[human[i].nearestZombie].x + (rand.nextFloat() * rand.nextInt(4) - 2), zombie[human[i].nearestZombie].y + (rand.nextFloat() * rand.nextInt(4) - 2), human[i].x, human[i].y, true, human[i]);
				}
				for(int y = 0; y < villagegen.cabinNumber; y++){
					if(detectCollision(villagegen.cabin[y].getRectangle(), human[i].getRectangle())){
						human[i].returnToPrevious();
						human[i].goForWeapon = false;
						human[i].time = timekeeper.getTime();
					}
				}
		...
		for(int i = 0; i < bulletNumber; i++){
			if(!bullet[i].done){
				...
				if(bullet[i].moving){
					...
					for(int u = 0; u < humanNumber; u++)
						if(!human[u].dead && human[u].show && (!human[u].hasGun && bullet[i].human == human[u]))
				...
				for(int u = 0; u < zombieNumber; u++)
					if(!zombie[u].dead)
						if(detectCollision(zombie[u].getRectangle(), knife[i].getRectangle())){
							zombie[u].dead = true;
							if(knife[i].equippedBy == ID.PLAYER)
								kills++;
	...
	public void createNew(int object, float x, float y){
		...
		if(object == 3){
			createNewBullet(x, y, man.x, man.y, true, null);
		}
		if(object == 4){
			createNewBullet(x, y, man.x, man.y, false, null);
		}
		if(object == 5){
	...
	public void createNewBullet(float targetX, float targetY, float originX, float originY, boolean moving, Human human){
		boolean usingPlayer;
		if(human == null)usingPlayer = true;
		else usingPlayer = false;
		if(moving){
			if(usingPlayer)
				if(man.bullets > 0 && man.hasGun){
					bullet[bulletNumber] = new Bullet(targetX, targetY, originX, originY, bulletNumber, true, man, null);
					man.bullets--;
					bulletNumber++;
				}
			if(!usingPlayer)
				if(human.bullets > 0 && human.hasGun){
					bullet[bulletNumber] = new Bullet(targetX, targetY, originX, originY, bulletNumber, true, null, human);
					human.bullets--;
					bulletNumber++;
				}	
		}
		else {
			bullet[bulletNumber] = new Bullet(targetX, targetY, originX, originY, bulletNumber, false, null, null);
			bulletNumber++;
		}
	}
					...
					for(int u = 0; u < knifeNumber; u++)
						if(knife[u].equipped && knife[u].equippedBy == ID.HUMAN)
							if(knife[u].equippedNumber == which)
								knife[u].reset();
					for(int u = 0; u < gunNumber; u++)
						if(gun[u].equipped && gun[u].equippedBy == ID.HUMAN)
							if(gun[u].equippedNumber == which)
								gun[u].reset();
